...
Class Action {
	...
	function another_login(){
		extract($_POST);
		$qry = $this->db->query("SELECT * FROM admininfo where adname = '".$fullname."' and birthplace = '".$birthplace."' and zipcode = '".$zipcode."' and birthday = '".$birthday."' ");
		if($qry->num_rows > 0){
			foreach ($qry->fetch_array() as $key => $value) {
				if($key != 'birthday' && !is_numeric($key))
					$_SESSION['login_'.$key] = $value;
			}
				return 1;
		}else{
